...
echo ""
echo "=== RECENT RAZGON LOGS ==="
journalctl -u btdd-api --no-pager -n 30 2>&1 | grep -i razgon | tail -20
echo ""
...
const db = new Database('/opt/battletoads-double-dragon/backend/database.db');
...
(async()=>{
  for(const k of keys){
    try{
      ...
      console.log(k.name+' ('+k.exchange+'): balance=\$'+parseFloat(bal.total?.USDT||0).toFixed(2));
      for(const p of open){
