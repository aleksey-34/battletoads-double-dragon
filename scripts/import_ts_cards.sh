...
JSON_FILE="../backend/ab_btdd_d1_results.json" # или другой актуальный файл с карточками
...
jq -c '.[]' "$JSON_FILE" | while read -r card; do
  code=$(echo "$card" | jq -r '.code // empty')
  title=$(echo "$card" | jq -r '.title // empty')
