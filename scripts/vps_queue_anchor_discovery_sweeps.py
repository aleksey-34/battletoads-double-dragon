...
QUEUE = [
    {
        ...
        "systemName": "ANCHOR MEXC Candidate",
        "strategyPrefix": "ANCHOR_MEXC",
    },
    {
        "apiKeyName": "HDB_17",
        "systemName": "ANCHOR Bitget Candidate",
        "strategyPrefix": "ANCHOR_BITGET",
    },
]
...
def build_payload(item: dict):
    ...
    return {
        ...
        "checkpointEvery": 20,
        "resumeEnabled": False,
        "checkpointFile": f"/opt/battletoads-double-dragon/results/{prefix.lower()}_anchor_checkpoint.json",
...
def main():
    print(json.dumps({"queue": QUEUE, "purpose": "anchor_discovery"}, ensure_ascii=False), flush=True)
    for item in QUEUE:
        wait_until_free()
        ...
        try:
            ...
            print(json.dumps({"queuedLaunch": item["apiKeyName"], "payload": payload, "httpError": exc.code, "body": exc.read().decode("utf-8")}, ensure_ascii=False), flush=True)
            break
