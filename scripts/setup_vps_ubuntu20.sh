...
MONITORING_SNAPSHOT_SEC="${MONITORING_SNAPSHOT_SEC:-300}"
RECONCILIATION_INTERVAL_MIN="${RECONCILIATION_INTERVAL_MIN:-360}"
RECONCILIATION_PERIOD_HOURS="${RECONCILIATION_PERIOD_HOURS:-24}"
RECONCILIATION_BACKTEST_BARS="${RECONCILIATION_BACKTEST_BARS:-336}"
RECONCILIATION_AUTO_APPLY="${RECONCILIATION_AUTO_APPLY:-0}"
RECONCILIATION_AUTO_PAUSE="${RECONCILIATION_AUTO_PAUSE:-0}"
LIQUIDITY_SCAN_INTERVAL_MIN="${LIQUIDITY_SCAN_INTERVAL_MIN:-180}"
LIQUIDITY_SCANNER_TOP_UNIVERSE="${LIQUIDITY_SCANNER_TOP_UNIVERSE:-120}"
NODE_MAJOR="${NODE_MAJOR:-20}"
ADMIN_PASSWORD="${ADMIN_PASSWORD:-}"
ENABLE_GIT_UPDATE="${ENABLE_GIT_UPDATE:-1}"
PASSWORD_STATE_FILE="${PASSWORD_STATE_FILE:-${APP_DIR}/backend/.auth-password.json}"
RECOVERY_TELEGRAM_BOT_TOKEN="${RECOVERY_TELEGRAM_BOT_TOKEN:-}"
RECOVERY_TELEGRAM_CHAT_ID="${RECOVERY_TELEGRAM_CHAT_ID:-}"
RECOVERY_CODE_TTL_MIN="${RECOVERY_CODE_TTL_MIN:-10}"
...
MONITORING_SNAPSHOT_SEC=${MONITORING_SNAPSHOT_SEC}
RECONCILIATION_INTERVAL_MIN=${RECONCILIATION_INTERVAL_MIN}
RECONCILIATION_PERIOD_HOURS=${RECONCILIATION_PERIOD_HOURS}
RECONCILIATION_BACKTEST_BARS=${RECONCILIATION_BACKTEST_BARS}
RECONCILIATION_AUTO_APPLY=${RECONCILIATION_AUTO_APPLY}
RECONCILIATION_AUTO_PAUSE=${RECONCILIATION_AUTO_PAUSE}
LIQUIDITY_SCAN_INTERVAL_MIN=${LIQUIDITY_SCAN_INTERVAL_MIN}
LIQUIDITY_SCANNER_TOP_UNIVERSE=${LIQUIDITY_SCANNER_TOP_UNIVERSE}
PASSWORD_HASH=${password_hash}
PASSWORD_STATE_FILE=${PASSWORD_STATE_FILE}
APP_DIR=${APP_DIR}
GIT_BRANCH=${BRANCH}
...
RECOVERY_TELEGRAM_BOT_TOKEN=${RECOVERY_TELEGRAM_BOT_TOKEN}
RECOVERY_TELEGRAM_CHAT_ID=${RECOVERY_TELEGRAM_CHAT_ID}
RECOVERY_CODE_TTL_MIN=${RECOVERY_CODE_TTL_MIN}
