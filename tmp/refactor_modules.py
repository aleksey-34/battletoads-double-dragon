#!/usr/bin/env python3
"""Fix and regenerate strategy modules on VPS, then patch strategy.ts."""
import os, shutil
...
###############################################################################
# 1. sizing.ts — extracted from strategy.js (compiled code)
###############################################################################
sizing_ts = '''// Sizing & Risk Management
// Extracted from bot/strategy.ts — handles notional, partial TP, SL, TP.
...
# 2. mutex.ts
###############################################################################
mutex_ts = '''// Strategy Mutex — race-condition prevention
...
###############################################################################
# 3. crud.ts (minimal — leave real CRUD in strategy.ts for now)
###############################################################################
crud_ts = '''// Strategy CRUD — thin re-exports, real implementation still in bot/strategy.ts
...
# 4. index.ts
###############################################################################
index_ts = '''// Strategy Services — unified exports
export { computeSignalTotalNotional, decimalPlaces, partialTpTriggeredByStrategy, computePartialTakeProfit, computeStopLoss, computeTakeProfit } from "./sizing";
